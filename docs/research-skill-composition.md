...
The framework is already well aligned with agentskills.io **progressive disclosure** (catalog → SKILL.md → `references/`). The main gap is **cross-mission orchestration** and a **consistent composition contract** across the shipped missions.
...
`npx skills` and `.agents/skills/` are the interoperability surface. autonomous-fleet publishes **12** skills under `skills/*/` (umbrella, program, setup, core, 5 adapters, 3 shipped missions; 12 more missions are exploratory under `docs/`); installs copy into `.agents/skills/`. Project skills can shadow user skills (project wins).
...
1. **Standardize `## Required skills`** on the shipped missions (match `doc-sync` template).
