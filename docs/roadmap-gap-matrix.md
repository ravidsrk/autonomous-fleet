...
| W3-trace11 | Trace schema 1.1 (optional id bump) | `fleet-trace.schema-1.1.json` + validate 1.0/1.1 | — | closed (v0.2.1) |
...
| H-headless | Headless E2E with runtime auth | `validate-headless.sh` + Lane 1/2 live proof | — | closed (Lanes 1–2 on `main`) |
| M-promote | 12 exploratory missions | Demoted to `docs/exploratory/` | PENDING: archive triple each | promotion criteria in matrix |
| D-starlight | Public docs site (Starlight) | `docs-site/` scaffold + sync + CI workflow | PENDING: Cloudflare Pages + domain | Stage 2.A shipped in repo |
