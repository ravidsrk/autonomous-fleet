...
def first_hook_signal_at(events: list[Event]) -> datetime | None:
...
def spawn_or_restore_at(events: list[Event], task_id: str | None = None) -> datetime | None:
    ...
    for event in events:
        if event.get("primitive") not in {"SPAWN_WORKER", "CONTINUE_WORKER"}:
            continue
        if task_id and (event.get("details") or {}).get("task_id") != task_id:
            continue
...
def verify_hook_signal_trace(
    ...
    *,
    ...
    task_ids = {
        (event.get("details") or {}).get("task_id")
        for event in events
        if (event.get("details") or {}).get("task_id")
    }
    ...
    for task_id in sorted(task_ids):
        ...
        if spawn_at is None:
            continue
        first_signal = first_hook_signal_at(
            [
                e
                for e in events
                if (e.get("details") or {}).get("task_id") == task_id
            ]
        )
        expected = derive_signal_status(
            spawn_at=spawn_at,
            now=now,
