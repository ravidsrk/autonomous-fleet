...
- The trace `primitive`/`role` enums are trace-specific vocabulary — overlapping with, but NOT
  identical to, the 13 coordinator PRIMITIVES (`engine-workers.md`). The trace records ledger state-transition verbs the
  ...
  other; the 13-primitive coordinator list in `engine-workers.md`'s THE PRIMITIVES section is unchanged.
